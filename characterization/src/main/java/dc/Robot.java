...
public class Robot extends TimedRobot {
  ...
  // static private double ENCODER_EDGES_PER_REV = 8192;
  ...
  public WPI_TalonFX setupWPI_TalonFX(int port, Sides side, boolean inverted) {
    ...
    if (side != Sides.FOLLOWER) {
      ...
      // CANCoder frontEncoder;
      // CANCoder backEncoder;
    ...
    switch (side) {
      ...
      case RIGHT:
        ...
        // frontEncoder = new CANCoder(17);
        // frontEncoder.configSensorDirection(false);
        // backEncoder = new CANCoder%(18);
        // backEncoder.configSensorDirection(false);

        // frontEncoder. setDistancePerPulse((double) encoderConstant / 4);
        rightEncoderPosition = () -> ((double) motor.getSelectedSensorPosition() / (2048.0 * 6.86)) * (1/3.0 * Math.PI) ;
        rightEncoderRate = () -> (((double) motor.getSelectedSensorVelocity() * 10) / (2048.0 * 6.86)) * (1/3.0 * Math.PI);

        break;
      case LEFT:
        // frontEncoder = new CANCoder(16);
        // frontEncoder.configSensorDirection(false);
        // backEncoder = new CANCoder(15);
        // backEncoder.configSensorDirection(false);

        // encoder.setDistancePerPulse((double) encoderConstant / 4);
        leftEncoderPosition = () -> ((double) motor.getSelectedSensorPosition() / (2048.0 * 6.86)) * (1/3.0 * Math.PI);
        leftEncoderRate = () -> (((double) motor.getSelectedSensorVelocity() * 10) / (2048.0 * 6.86)) * (1/3.0 * Math.PI);
  ...
  public void robotInit() {
    ...
    // create left motor
    WPI_TalonFX leftMotor = setupWPI_TalonFX(8, Sides.LEFT, false);

    ArrayList<SpeedController> leftMotors = new ArrayList<SpeedController>();
    leftMotors.add(setupWPI_TalonFX(10, Sides.FOLLOWER, false));
    ...
    rightMotors.add(setupWPI_TalonFX(13, Sides.FOLLOWER, false));
    ...
    PigeonIMU pigeon = new PigeonIMU(19);
    gyroAngleRadians = () -> {
